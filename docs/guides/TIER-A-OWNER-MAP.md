...
|---|---|---|---|---|---|
| Runtime entry + locale routing | `src/middleware.ts`, `src/i18n/**`, `src/app/[locale]/layout.tsx`, `src/app/[locale]/head.tsx` | Runtime / i18n maintainer | Platform maintainer | locale redirect / locale cookie 改动时补 Security review | 普通合并至少 `local-full`，release-critical 走 `release-proof` |
