...
- Semgrep security scan: CI runs `semgrep scan --error --config semgrep.yml src` in the official `semgrep/semgrep` container. A missing local `semgrep` binary is `Blocked`, not `Passed`.
