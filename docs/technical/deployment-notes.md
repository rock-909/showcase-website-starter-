...
- Cloudflare preview smoke：`pnpm smoke:cf:preview`
- Deployed smoke：`pnpm smoke:cf:deploy -- --base-url <url>`
- Cloudflare dry-run：`pnpm deploy:cf:dry-run`
...
`pnpm build` 和 `pnpm build:cf` 都会使用 `.next` 这一类构建产物，不要并行运行。
...
如果改到 Cloudflare / OpenNext / middleware / deploy scripts，再补 `pnpm build:cf` 和对应 smoke。

## Cloudflare Topology

当前仓库保留 split-worker 部署脚本。默认命令：
...
如果要把 `compatibility_date` 继续推进到 `2026-05-07` 或更晚日期，需要按 Cloudflare runtime 变更处理，至少重新跑 `pnpm build`、`pnpm build:cf`，并按是否涉及上线补 preview smoke。
...
`pnpm build:cf` 通过不等于页面可访问。  
`pnpm smoke:cf:deploy` 通过也不等于真实询盘链路已闭环。
