...
| 本地发布证明 | 证明当前机器上能完成生产构建 | `pnpm build`、`pnpm build:cf`、`pnpm release:verify` | CI 环境和线上环境一定可用 |
...
注意：如果页面受 Vercel preview protection 保护，proof 必须接在部署后等待就绪之后，并复用现有 bypass header。不要在部署 URL 刚输出时提前探测，否则容易制造假红灯。
...
- `/api/health` 返回 2xx 或 3xx。
- 响应带 `x-request-id`。
- 响应带明确的 observability surface，例如 `x-observability-surface`。
...
Next.js 当前推荐 `proxy.ts`，但 Cloudflare/OpenNext 是否能稳定跑，需要单独证明。  
不要在派生项目里因为提示 deprecated 就直接把 `src/middleware.ts` 改名。
