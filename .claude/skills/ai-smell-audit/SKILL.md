...
Security note: `security-scanning`-style work is handled here as a skill lane plus existing repo scripts (`security:semgrep`, `security:audit`, `security:csp:check`, `lint:pii`, `review:*env-boundaries`) when available. Do not install or invoke a separate generic security plugin unless the user explicitly asks.
